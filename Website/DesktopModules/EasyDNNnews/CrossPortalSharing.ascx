<%@ control language="C#" inherits="EasyDNNSolutions.Modules.EasyDNNNews.Administration.CrossPortalSharing, App_Web_crossportalsharing.ascx.d988a5ac" autoeventwireup="true" %>
...
<div class="edNews_adminWrapper mainContentWrapper topPadded bottomPadded">
	<div class="contentSection bottomPadded">
		<div class="titleWrapper">
			...
			<span><%=CrossPortalsharing%></span>
		</div>
		<div class="sectionBox sectionBox2">
			<div class="sectionBoxSubtitle textLeft">
				<asp:Label ID="lblPortalSharing" runat="server" Text="Portal sharing options" resourcekey="lblPortalSharing"></asp:Label>
			</div>
			<div class="portal_sharing_container">
				...
				<asp:GridView ID="gvPortalSharing" runat="server" AutoGenerateColumns="False" GridLines="None" CssClass="fullWidthTable optionsList strippedTable" resourcekey="gvPortalSharingResource1" OnRowDataBound="gvPortalSharing_RowDataBound">
					<Columns>
						<asp:TemplateField HeaderText="Portal">
							<ItemTemplate>
								<asp:Label ID="Label1" runat="server" Text='<%# String.Format("{0} (Id:{1})", Eval("PortalName"), Eval("PortalID")) %>' resourcekey="Label1Resource1"></asp:Label>
							</ItemTemplate>
							<HeaderStyle CssClass="textRight"></HeaderStyle>
							<ItemStyle CssClass="tdLabel textTop"></ItemStyle>
						</asp:TemplateField>
						<asp:TemplateField HeaderText="Shared on portals">
							<ItemTemplate>
								<asp:HiddenField ID="hfPortalID" runat="server" Value='<%# Eval("PortalID") %>' />
								<asp:RadioButton ID="rbPortalsSelect" runat="server" CssClass="permissions_show_manual_item_selection styledRadio smallRadio" GroupName="portalsShareSelection" Text="Select portals" resourcekey="rbPortalsSelectResource1" />
								<asp:RadioButton ID="rbPortalsNone" runat="server" Checked='True' CssClass="permissions_show_no_items styledRadio smallRadio" GroupName="portalsShareSelection" Text="None" resourcekey="rbPortalsNoneResource1" />
								<asp:Panel runat="server" ID="pnlAddToCatsManualSelection" Style="display: none;" CssClass="permissions_manual_item_selection" resourcekey="pnlAddToCatsManualSelectionResource1">
									...
									<p>
										<asp:LinkButton ID="lbPortalsToShareTo" runat="server" CssClass="permissions_show_selection_dialog" Text="Select portals" resourcekey="lbPortalsToShareToResource1" /></p>
				...
				<div class="mainActions">
					<asp:Button ID="btnSaveSettings" CssClass="downSave" runat="server" OnClick="btnSaveSettings_Click" Text="Save" resourcekey="btnSaveSettingsResource1" />
					<asp:Button ID="btnSaveClose" CssClass="downSaveClose" runat="server" OnClick="btnSaveClose_Click" Text="Save &amp; Close" resourcekey="btnSaveCloseResource1" />
					<asp:Button ID="btnClose" CssClass="downClose" resourcekey="btnClose" runat="server" Text="Close" OnClick="btnClose_Click" />
				</div>
			</div>
			<asp:Label ID="lblPortalSharingMessage" runat="server" Visible="False" resourcekey="lblPortalSharingMessageResource1">Settings saved.</asp:Label>
		</div>
	</div>
</div>
