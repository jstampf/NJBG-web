											...
											<ItemTemplate>
												...
												<asp:PlaceHolder ID="plAudioinFlash" runat="server" Visible='<%#!Html5%>'>
													<div id='pid<%=ModulNr%><%#Eval("PictureID")%>' class="AudioBody">
														<audio src="<%#Server.HtmlEncode(Eval("Filename").ToString())%>" preload="none" <%=myappSettings.AudioPlayOnLoad%>></audio>
													</div>
												</asp:PlaceHolder>
												<asp:PlaceHolder ID="plHTML5Audio" runat="server" Visible='<%#Html5%>'>
													<audio controls preload="none" <%=myappSettings.AudioPlayOnLoad%>> 
