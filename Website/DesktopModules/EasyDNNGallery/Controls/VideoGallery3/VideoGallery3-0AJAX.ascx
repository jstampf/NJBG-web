<%@ control language="C#" autoeventwireup="true" inherits="EasyDNN.Modules.EasyDNNGallery.DesktopModules_EasyDNNGallery_Controls_VideoGallery3ControlAJAX, App_Web_videogallery3-0ajax.ascx.e651496d" %>
<style type="text/css">
	.titleempty
	{
		display:none;
	...
	function SetupSocialSharing()
		...
		function InsertSWF(fileName,vidWidth, vidHeight)
		{
			 <%=jQuery%>(document).ready(function() {
						...
						var attributes = {};
						swfobject.embedSWF(fileName, '<%=ModulNr%>swf', vidWidth, vidHeight, "9.0.0",'<%=ModulePath.Replace("Controls/VideoGallery3/","")%>js/expressInstall.swf', flashvars, params, attributes);
					}); 
		}
	   ...
	   function InsertFP(vfilename,sphoto,vidwidth1,vidheight1,vidwidth2,vidheight2,idesc)
		{
			<%=jQuery%>(document).ready(function()
				...
				attributes.id = 'pp'
				/* ]]> */                
				swfobject.embedSWF('<%=ModulePath.Replace("Controls/VideoGallery3/","")%>preview.swf', '<%=ModulNr%>', vidwidth2, vidheight2, '9.0.0', '<%=ModulePath.Replace("Controls/VideoGallery3/","")%>js/expressInstall.swf', flashvars, params, attributes);
				});
	   }
		 ...
		 <%=jQuery%>(document).ready(function($) {		 
			...
			if ($.browser.msie && <%=jQuery%>.browser.version < "8.0") {
				...
				<%=jQuery%>(window).load(function($) {
					edgcar =  <%=jQuery%>('#IC<%=ModulNr%>').carouFredSel({
					auto : <%=SSAutoSlideRandom%>,
					auto : {
					play:<%=myappSettings.SSAutoSlide%>,
					...
					pagination	: '#<%=myappSettings.PagingVisible%>'
				});
				<%=jQuery%>('#<%=CssClass%> #IC<%=ModulNr%> .EDGmain').css('visibility', 'visible');
				<%=jQuery%>('.image_carousel').css('overflow', 'visible');
				<%=jQuery%>('.image_carousel').css('height', 'auto');
					var hiddenControl = '<%= HiddenField1.ClientID %>';
					itmpos = document.getElementById(hiddenControl).value;
					<%=jQuery%>('#IC<%=ModulNr%>').trigger("slideTo", parseInt(itmpos));
				});
				...
				var hiddenControl = '<%= HiddenField1.ClientID %>';
				itmpos = document.getElementById(hiddenControl).value;
				$('#IC<%=ModulNr%>').trigger("slideTo", parseInt(itmpos));
				$('#<%=CssClass%> #IC<%=ModulNr%> .EDGmain').css('visibility', 'visible');
				$('.image_carousel').css('overflow', 'visible');
				$('.image_carousel').css('height', 'auto');
				$.each($.browser, function (i) {
																...
																<ItemTemplate>
																	...
																	<asp:PlaceHolder ID="phEVVideo" runat="server" Visible='<%# ToDisplayVideo2Flow("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName"),"Normal")%>'>
																		...
																		<%=jQuery%>(document).ready(function() {
																			...
																			attributes.id = 'pp'
																			/* ]]> */                
																			swfobject.embedSWF('<%=ModulePath.Replace("Controls/VideoGallery3/","")%>preview.swf', '<%=ModulNr%>', '<%#GetVideoWidth(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>', '<%# GetVideoHeight(DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "FileName"))%>', '9.0.0', '<%=ModulePath.Replace("Controls/VideoGallery3/","")%>js/expressInstall.swf', flashvars, params, attributes);
																		});
																		</script>
																	</asp:PlaceHolder>
																	<asp:PlaceHolder ID="phEVVideoFlow" runat="server" Visible='<%# ToDisplayVideo2Flow("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName"),"Flow") %>'>
																		...
																		<div class="VideoDescription">
																			<asp:Label ID="Label4" runat="server" Text='<%# ShowDescription(DataBinder.Eval(Container.DataItem, "Description"),DataBinder.Eval(Container.DataItem, "PictureID"))%>' Visible="<%#Convert.ToBoolean(myappSettings.ShowMediaDescription)%>"></asp:Label>
																		</div>
																		<div>
																			<br />
																		</div>
																	...
																	<asp:PlaceHolder ID="phHtml5Video" runat="server" Visible='<%# ToDisplayVideoHtml5("Video",DataBinder.Eval(Container.DataItem, "MediaType"),DataBinder.Eval(Container.DataItem, "Info"),DataBinder.Eval(Container.DataItem, "FileName")) %>'>
																		...
																		<%=jQuery%>(document).ready(function(){
																			<%=jQuery%>("#Divvideo_player").H5Video({
																				...
																				supportMessage : "This browser cannot playback HTML5 videos. We encourage you to upgrade your internet browser."
																			});
																		});
																...
																<tr>
																	<td>
																		<div class="image_carousel">
																			...
																			<a class="prev<%=ModulNr%>" id='foo2_prev<%=ModulNr%>' href="#" style='visibility: visible'><span>prev</span></a> <a class="next<%=ModulNr%>" id='foo2_next<%=ModulNr%>' href="#"
																				style='visibility: visible'><span>next</span></a>
